module cla_4bit_behavioural_tb();
  reg [3:0]a;
  reg [3:0]b;
  reg cin;
  wire[3:0]s;
  wire cout;
   cla_4bit_behavioural dut(.a(a),.b(b),.cin(cin),.s(s),.cout(cout));
  initial begin    
  a=4'b0000;b=4'b0000;cin=1'b0;
    #10 a=4'b1001;b=4'b0010;cin=1'b1;
    #10 a=4'b0011;b=4'b0110;cin=1'b0;
    #10 a=4'b0111;b=4'b1110;cin=1'b1;
    #10 a=4'b1011;b=4'b0101;cin=1'b0;
    #10 a=4'b1010;b=4'b1010;cin=1'b1;
    #10 a=4'b1100;b=4'b0000;cin=1'b0;
    #10 a=4'b1110;b=4'b1000;cin=1'b0;
    #10 a=4'b1100;b=4'b0100;cin=1'b0;
    #10 a=4'b0110;b=4'b1011;cin=1'b0;
    #10 a=4'b0100;b=4'b1101;cin=1'b0;
    #10 $finish;
  end
  initial begin
    $monitor(" time=%0t  ||A=%b B=%b Cin=%b ||SUM=%b  Cout=%b",$time,a,b,cin,s,cout);
  end
  initial begin
    $dumpfile("cla_4bit_behavioural.vcd");
    $dumpvars;
  end
endmodule
